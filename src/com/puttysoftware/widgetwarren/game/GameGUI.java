...
class GameGUI {
	...
	if (this.outputFrame.isVisible()) {
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
			...
			if (visible) {
			    ...
			    obj1 = app.getMazeManager().getMaze().getCell(y, x, z, MazeConstants.LAYER_GROUND);
			    final BufferedImageIcon icon1 = ObjectImageManager.getObjectImage(obj1,
				    !this.trueSightFlag);
			    obj2 = app.getMazeManager().getMaze().getCell(y, x, z, MazeConstants.LAYER_OBJECT);
			    if (obj2.getSavedObject() != null) {
				final BufferedImageIcon icon2 = ObjectImageManager.getObjectImage(obj2.getSavedObject(),
					!this.trueSightFlag);
				final BufferedImageIcon icon3 = ObjectImageManager.getObjectImage(obj2,
					!this.trueSightFlag);
				this.drawGrid[xFix][yFix].setIcon(ImageModifier.getCompositeImage(icon1, icon2, icon3));
			    } else {
				final BufferedImageIcon icon2 = ObjectImageManager.getObjectImage(obj2,
					!this.trueSightFlag);
				this.drawGrid[xFix][yFix].setIcon(ImageModifier.getCompositeImage(icon1, icon2));
			    }
			} else {
			    this.drawGrid[xFix][yFix]
				    .setIcon(ObjectImageManager.getObjectImage(new Darkness(), !this.trueSightFlag));
			}
		    } catch (final ArrayIndexOutOfBoundsException ae) {
			this.drawGrid[xFix][yFix]
				.setIcon(ObjectImageManager.getObjectImage(new EmptyVoid(), !this.trueSightFlag));
    ...
    void redrawOneSquare(final int x, final int y, final boolean useDelay, final MazeObject obj3, final int u,
	    final int v, final int z) {
	...
	if (this.outputFrame.isVisible()) {
	    ...
	    visible = app.getMazeManager().getMaze().isSquareVisible(u, v, x, y);
	    try {
		if (visible) {
		    MazeObject obj1, obj2;
		    obj1 = app.getMazeManager().getMaze().getCell(x, y, z, MazeConstants.LAYER_GROUND);
		    final BufferedImageIcon icon1 = ObjectImageManager.getObjectImage(obj1, !this.trueSightFlag);
		    obj2 = app.getMazeManager().getMaze().getCell(x, y, z, MazeConstants.LAYER_OBJECT);
		    final BufferedImageIcon icon2 = ObjectImageManager.getObjectImage(obj2, !this.trueSightFlag);
		    final BufferedImageIcon icon3 = ObjectImageManager.getObjectImage(obj3, !this.trueSightFlag);
		    this.drawGrid[xFix][yFix].setIcon(ImageModifier.getCompositeImage(icon1, icon2, icon3));
		} else {
		    this.drawGrid[xFix][yFix]
			    .setIcon(ObjectImageManager.getObjectImage(new Darkness(), !this.trueSightFlag));
	...
	public void handleMovement(final KeyEvent e) {
		...
		case KeyEvent.VK_ENTER:
		    if (e.isShiftDown() && !gm.usingAnItem() && !gm.isTeleporting()) {
			gm.updatePositionRelative(0, 0);
		    }
		    break;
