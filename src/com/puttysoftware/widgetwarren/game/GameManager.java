...
public class GameManager implements MazeEffectConstants {
	...
	this.lastUsedBowIndex = WidgetWarren.getApplication().getObjects().getAllBowNames().length - 1;
	...
	final MazeObject acted = WidgetWarren.getApplication().getMazeManager().getMazeObject(x, y, z,
		MazeConstants.LAYER_OBJECT);
	...
	do {
		...
		try {
		    if (this.checkSolid(fX + px, fY + py, this.player.getSavedObject(), below, nextBelow, nextAbove)) {
			if (this.delayedDecayActive) {
			    this.doDelayedDecay();
			}
			m.setCell(this.player.getSavedObject(), px, py, pz, MazeConstants.LAYER_OBJECT);
			acted = new Empty();
			try {
			...
			this.player.setSavedObject(m.getCell(px, py, pz, MazeConstants.LAYER_OBJECT));
			m.setCell(this.player, px, py, pz, MazeConstants.LAYER_OBJECT);
			this.decayEffects();
			...
			if (groundInto.overridesDefaultPostMove()) {
			    ...
			    if (!this.player.getSavedObject().isOfType(TypeConstants.TYPE_PASS_THROUGH)) {
				this.player.getSavedObject().postMoveAction(false, px, py, this.objectInv);
			    }
			} else {
			    this.player.getSavedObject().postMoveAction(false, px, py, this.objectInv);
			}
			...
			if (acted.isPushable()) {
			    ...
			    this.fireMoveFailedActions(px + fX, py + fY, this.player.getSavedObject(), below, nextBelow,
				    nextAbove);
			    this.decayEffects();
			    this.fireStepActions();
			    proceed = false;
	...
	if (this.objectInv.isItemThere(new SlipperyBoots()) || this.objectInv.isItemThere(new IceAmulet())) {
	    return proceed && this.checkSolid(x, y, this.player.getSavedObject(), below, nextBelow, nextAbove);
	} else {
	    return proceed && !groundInto.hasFrictionConditionally(this.objectInv, false)
		    && this.checkSolid(x, y, this.player.getSavedObject(), below, nextBelow, nextAbove);
	}
    }
	...
	this.player.setSavedObject(m.getCell(opx, opy, pz, MazeConstants.LAYER_OBJECT));
	...
	try {
	    ...
	    return this.checkSolid(x, y, this.player.getSavedObject(), below, nextBelow, nextAbove);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	}
    }
	...
	try {
	    ...
	    return this.checkSolidAbsolute(this.player.getSavedObject(), below, nextBelow, nextAbove);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	}
    }
	...
	try {
		...
		m.setCell(this.player.getSavedObject(), this.plMgr.getPlayerLocationX(),
			this.plMgr.getPlayerLocationY(), this.plMgr.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
		this.plMgr.setPlayerLocation(x, y, z, 0);
		this.getViewManager().setViewingWindowLocationX(
			this.plMgr.getPlayerLocationY() - this.getViewManager().getOffsetFactorX());
		this.getViewManager().setViewingWindowLocationY(
			this.plMgr.getPlayerLocationX() - this.getViewManager().getOffsetFactorY());
		...
		this.redrawMaze();
		app.getMazeManager().setDirty(true);
		this.player.getSavedObject().postMoveAction(false, x, y, this.objectInv);
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    this.plMgr.restorePlayerLocation();
	    this.getViewManager().restoreViewingWindow();
	    m.setCell(this.player, this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
		    this.plMgr.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
	    WidgetWarren.getApplication().showMessage("Can't go outside the maze");
	} catch (final NullPointerException np) {
	    this.plMgr.restorePlayerLocation();
	    this.getViewManager().restoreViewingWindow();
	    m.setCell(this.player, this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
	...
	try {
		...
		m.setCell(p, this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
			this.plMgr.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
		this.redrawMaze();
		app.getMazeManager().setDirty(true);
	    }
	...
	final Maze m = WidgetWarren.getApplication().getMazeManager().getMaze();
	m.resetTimer();
	SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE, SoundConstants.SOUND_WIN_GAME);
    ...
    public void decay() {
	if (this.actingRemotely) {
	    WidgetWarren.getApplication().getMazeManager().getMaze().setCell(new Empty(), this.remoteCoords[0],
	...
	if (this.actingRemotely) {
	    WidgetWarren.getApplication().getMazeManager().getMaze().setCell(decay, this.remoteCoords[0],
		    this.remoteCoords[1], this.remoteCoords[2], MazeConstants.LAYER_OBJECT);
	} else {
	...
	if (this.actingRemotely) {
	    WidgetWarren.getApplication().getMazeManager().getMaze().setCell(this.delayedDecayObject,
	...
	final String result = CommonDialogs.showInputDialog("Use which item?", "WidgetWarren", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			WidgetWarren.getApplication().showMessage("That item has no more uses left.");
			this.setUsingAnItem(false);
		    } else {
			WidgetWarren.getApplication().showMessage("Click to set target");
		    }
		    return;
		}
	    }
	...
	final String result = CommonDialogs.showInputDialog("Switch to which bow?", "WidgetWarren", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
		    ...
		    if (this.objectInv.getUses(this.activeBow) == 0) {
			WidgetWarren.getApplication().showMessage("That bow is out of arrows!");
		    } else {
			WidgetWarren.getApplication().showMessage(this.activeBow.getName() + " activated.");
		    }
		    return;
		}
	    }
	...
	final int destX = x / ImageConstants.getImageSize() + this.getViewManager().getViewingWindowLocationX()
		- xOffset + yOffset;
	final int destY = y / ImageConstants.getImageSize() + this.getViewManager().getViewingWindowLocationY()
		+ xOffset - yOffset;
	...
	if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
		...
		if ((target.isSolid() || !visible) && name.equals(new TeleportWand().getName())) {
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Can't teleport there");
		}
		if (target.getName().equals(this.player.getName())) {
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Don't aim at yourself!");
		}
		if (!target.isDestroyable() && name.equals(new AnnihilationWand().getName())) {
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Can't destroy that");
		}
		if (!target.isDestroyable() && name.equals(new WallMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Can't create a wall there");
		}
		if (!target.isDestroyable() && name.equals(new FinishMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Can't create a finish there");
		}
		...
		if (!target.getName().equals(new Empty().getName()) && !target.getName().equals(new DarkGem().getName())
		    ...
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Aim at either an empty space or a Dark Gem");
		}
		if (!target.getName().equals(new Empty().getName())
			&& !target.getName().equals(new LightGem().getName())
			&& name.equals(new DarkWand().getName())) {
		    this.setUsingAnItem(false);
		    WidgetWarren.getApplication().showMessage("Aim at either an empty space or a Light Gem");
		}
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		this.setUsingAnItem(false);
	...
	try {
	    ...
	    WidgetWarren.getApplication().showMessage(gameName2 + " on " + gameName1);
