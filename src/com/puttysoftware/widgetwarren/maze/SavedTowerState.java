...
class SavedTowerState implements Cloneable {
    ...
    public static SavedTowerState readSavedTowerStateXML(final XDataReader reader, final int formatVersion)
	    throws IOException {
	...
	for (x = 0; x < sts.c; x++) {
		...
		for (z = 0; z < sts.f; z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			sts.saveData[x][y][z][e] = WidgetWarren.getApplication().getObjects().readMazeObjectXML(reader,
				formatVersion);
		    }
		}
	    }
