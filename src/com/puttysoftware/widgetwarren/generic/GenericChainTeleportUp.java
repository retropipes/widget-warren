...
public abstract class GenericChainTeleportUp extends GenericTeleportUp {
	...
	return this.searchNestedSprings(dirX, dirY,
		WidgetWarren.getApplication().getGameManager().getPlayerManager().getPlayerLocationZ() + 1, inv);
    }
	...
	try {
	    ...
	    SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE, SoundConstants.SOUND_SPRINGBOARD);
	    WidgetWarren.getApplication().getMazeManager().getMaze().setCell(new Empty(), x, y, z,
		    MazeConstants.LAYER_OBJECT);
	}
    }
