...
public abstract class GenericContainer extends GenericLock {
	...
	if (!this.inside.getName().equals("Empty")) {
	    WidgetWarren.getApplication().showMessage(this.getName() + ": Contains " + this.inside.getName());
	} else {
	    WidgetWarren.getApplication().showMessage(this.getName() + ": Contains Nothing");
	}
    }
	...
	final MazeObjectList objectList = WidgetWarren.getApplication().getObjects();
